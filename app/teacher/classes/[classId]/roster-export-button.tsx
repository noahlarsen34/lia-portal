...
import { exportRosterToGoogleSheet } from "./roster-export-actions";

type RosterExportButtonProps = {
    classId: string;
    disabled?: boolean;
};
...
export function RosterExportButton({
    ...
    async function handleExport() {
        ...
        try {
            const result = await exportRosterToGoogleSheet(classId);
            ...
            setExportError(
                error instanceof Error
                    ? error.message
                    : "The student roster could not be exported.",
            );
        } finally {
